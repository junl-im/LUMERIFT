...
export function createResourceChip(
  ...
  const valueText = new Text({
    text: value,
    style: new TextStyle({ fill: 0xf4dca0, fontSize: 16, fontWeight: '700' }),
  });
...
export function createMenuTile(options: MenuTileOptions): Container {
  ...
  const label = new Text({
    text: options.label,
    style: new TextStyle({ fill: options.active ? 0xf8e6b1 : COLORS.text, fontSize: options.subtitle ? 15 : 12, fontWeight: '700' }),
  });
  ...
  if (options.subtitle) {
    const subtitle = new Text({
      ...
      style: new TextStyle({ fill: COLORS.muted, fontSize: 9, wordWrap: true, wordWrapWidth: width - 62 }),
    });
