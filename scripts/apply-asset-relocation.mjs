...
for (const planFile of planFiles) {
  const plan = JSON.parse(await readFile(`asset_registry/${planFile}`, 'utf8'));
...
if (errors.length) {
  ...
  console.log(`PASS asset relocation: plans ${planFiles.length}, moved ${moved}, already archived ${already}, removed ${removed}`);
}
