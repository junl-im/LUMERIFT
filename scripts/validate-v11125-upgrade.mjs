...
const sourceTokens = {
  'src/core/presentation/CharacterAppearanceConflictResolver.ts': [
    ...
    "lockedSlots: 'union'",
    "presets: 'merge'",
    'if (local.lockedSlots[slot])',
  ],
  'src/services/cloud/CharacterAppearanceRecoveryStore.ts': [
    'MAX_POINTS = 5',
    "'pre-conflict-merge'",
    "'pre-recovery-restore'",
    "'lumerift-character-appearance-recovery-archive-v1'",
    'record.ownerUid !== uid',
  ],
  ...
  'src/core/presentation/CharacterWardrobeController.ts': [
    'replacePresetArchive',
    'recentPresets: [...archive.presets]',
  ],
  'src/scenes/CharacterAppearanceConflictScene.ts': [
    '선택 내용 병합·Cloud 저장',
    "'pre-conflict-merge'",
    'CharacterAppearanceCloudScene',
  ],
  'src/scenes/CharacterAppearanceRecoveryScene.ts': [
    '현재 상태 수동 백업',
    '선택 지점 복구',
...
if (packageJson.version !== '1.11.25') errors.push(`package version ${packageJson.version} != 1.11.25`);
...
if (state.version !== '1.11.25') errors.push('HANDOFF_STATE version mismatch');
...
if (state.featureMetrics?.appearanceRecoveryMaxPoints !== 5) errors.push('appearance recovery point metric mismatch');
...
if (state.assetMetrics?.v11125NewRuntimeImageFiles !== 0) errors.push('v1.11.25 must not claim new runtime images');

const brand = await read('src/app/brand.ts');
if (!brand.includes("version: '1.11.25'")) errors.push('brand version mismatch');
...
if (assetManifest.release !== '1.11.25') errors.push('asset manifest release mismatch');

if (errors.length) {
  console.error(errors.join('\n'));
  process.exitCode = 1;
} else {
  console.log('PASS v1.11.25 upgrade: granular appearance conflict merge, protected slots, and UID-isolated recovery points');
}
