...
- 사용자 보고의 파일 용량은 십진 단위 `MB`로 표기한다. `MB`는 기술 참고가 꼭 필요한 경우에만 병기한다.
- **전체 통합 ZIP에는 코드·문서·런타임 자산·고해상도 원본·라이선스·레거시 보관 자산을 모두 포함한다.**
- 전체 통합 ZIP에서 원본이나 기존 자산을 임의로 삭제해 용량을 축소하지 않는다.
...
- `art_source`는 전체 통합본에 보존하지만 `public` 밖에 두어 GitHub Pages 배포와 초기 다운로드에는 포함하지 않는다.
