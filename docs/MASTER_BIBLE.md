...
첫 화면부터 계정·전투까지 동일한 루멘 판타지 인터페이스를 사용한다. 심해색 패널, 청록 상호작용, 저채도 금색 장식, 은백색 본문을 기본으로 하며 장식보다 상태와 행동의 판독성을 우선한다. 공통 Atlas와 NineSlice 컴포넌트를 우선하고 장면별 임시 사각형 UI의 증식을 금지한다.


## v1.11.1 전투 모션·기기 보정 원칙

- 기기 보정은 시각 비용만 바꾸며 충돌·피해·AI·쿨다운 판정을 바꾸지 않는다.
- 플레이어 모션 보정은 기존 8방향 Atlas 키와 피벗 계약을 유지한다.
- 보스 위험 장판은 색상 외에 기호·패턴명·단계·시간 정보를 함께 제공한다.
- 멀티터치 입력은 단일 포인터 소유권으로 중복 행동을 방지한다.
- 제작용 블록아웃은 최종 원화 승인 전까지 런타임 기본 자산으로 승격하지 않는다.
