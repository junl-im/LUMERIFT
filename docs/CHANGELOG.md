...
### Added
- 로컬·Cloud·RESULT 실제 캐릭터 3열 외형 충돌 미리보기
- 선택 병합 최종 Archive 시뮬레이션과 슬롯별 실제 적용 출처 표시
- 30분·1회성 외형 병합 Undo
- 복구 지점 이름 변경·검색·고정

### Changed
- 외형 복구 저장소를 v2로 확장하고 v1 자동 마이그레이션 유지
- 계정별 보관 한도를 고정 3개와 최근 5개로 분리
- 충돌 병합 적용 경로를 공통 Coordinator로 통합

### Known limitations
- Android·iOS 실제 캡처 승인값은 제공되지 않았다.
- 공격 본체와 장비 파츠는 최종 수작업 원화가 아니다.
- 내부 npm 저장소 404로 실제 Vitest·Vite production build는 미완료다.

## v1.11.27 - 2026-07-31

### Added
- 복구 지점 POINT A·POINT B 실제 캐릭터 2열 비교
- 슬롯·순서·고정·최근 프리셋 Archive diff v1
- 사용자 UID별 외형 감사 기록과 최대 100건 보관
- 선택 복구 지점 관련 차이·감사 기록 JSON 내보내기

### Changed
- Cloud 동기화·업로드·충돌 병합·Undo와 복구 관리 작업을 감사 기록에 연결
- v1.11.26 누적 검증기의 상위 버전 호환 보완

### Known limitations
- 실제 Android/iOS 물리 기기 캡처와 최종 수작업 원화는 미완료다.
- 실제 npm 의존성 기반 Vitest·Vite production build는 내부 저장소 404로 미완료다.

## v1.11.28 · Premium Art Direction v2

- 승인 캐릭터·몬스터 품질 쇼케이스 2종 추가
- 에셋 보관소 승인 기준 분류와 Lazy Reference 번들 추가
...
## v1.11.32 · 2026-08-03
- 플레이어·엘리트·보스 투명 WebP 래스터 파츠 Atlas 첫 배치
- 보스 코어 5상태 12프레임 FX와 Premium UI Icons 16프레임 적용
- 전투·아틀리에·인벤토리 v16 우선 로딩 및 기존 레이어 폴백
- 최종 전신 수작업 Atlas와 물리 기기 승인은 미완료 유지

## v1.11.33 · 2026-08-03

- 8방향 플레이어 래스터 파츠 32프레임과 방향별 피벗·가림 순서 적용
- 무기 3계열 공격 프레임별 파츠 위치 보정
- 엘리트·보스 전신 확장 파츠 24프레임 적용
- 보스 코어 FX 24프레임과 안정 상태 폴백 적용
- Premium UI Icons v17 24프레임 적용
- Player Save v4·AttackFootprint·App Check 비활성 정책 유지

## v1.11.34 · 2026-08-03

- 플레이어 동작 전용 v18 Atlas 48프레임·24애니메이션 추가
- 엘리트·보스 모션 v18 Atlas 32프레임·16애니메이션 추가
- 보스 코어 FX v18 30프레임·5애니메이션 추가
- Premium UI Icons v18 16프레임·16애니메이션 추가
- 전투·캐릭터 스튜디오·인벤토리 v18 우선 로딩과 v17 이하 폴백
- 최종 8방향 전신 수작업 Atlas와 물리 기기 승인은 미완료 유지
