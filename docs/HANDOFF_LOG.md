...
## 2026-07-31 · v1.11.19
- 기본 전투 플레이어를 프리미엄 8방향 본체 Atlas v10으로 전환했다.
- Atlas는 648프레임·80애니메이션이며 기존 `player.<state>.<direction>` 계약을 유지한다.
- 장비 등급에 따라 균열검·대검·균열 장창 프로그램 실루엣을 표시한다.
- 4종 캐릭터 염색 프리셋을 로컬 저장하고 로비·전투 재질에 연결했다.
- 레거시 v4 플레이어 Atlas는 비상 fallback과 라이선스 근거로 보존했다.
- 신규 본체는 production-candidate 파생 자산이며 수작업 최종 원화로 주장하지 않는다.


## 2026-07-31 · v1.11.20
- 캐릭터·코스튬 아틀리에를 추가하고 v10 본체 8개 포즈를 실제 AnimatedSprite로 재생했다.
- 염색과 포즈를 Player Save v4와 분리한 로컬 외형 슬롯 3개에 저장·복원할 수 있게 했다.
- 균열검·대검·균열 장창별 공격·스킬 모션 프로필을 전투 진입 시 적용했다.
- 로비 캐릭터·영웅 메뉴를 아틀리에로 연결하고 장비 보관소 이동을 제공했다.

## 2026-07-31 · v1.11.21

- 캐릭터 스튜디오 8방향 수동 회전과 방향별 v10 본체 프레임 연결
- 무기·방어구·장신구 슬롯별 현재/교체 후 외형 동시 비교
- 정찰대 강철·감시자 균열·전령 계승 코스튬과 갑주·망토·룬 세부 염색
- 검·대검·장창별 본체 프레임 레시피와 선행·접촉·회수 타이밍 분리
...
- 로컬·Cloud·RESULT 캐릭터를 나란히 보여주는 3열 외형 충돌 미리보기를 추가했다.
- 선택 병합을 저장하기 전에 슬롯별 실제 적용 출처와 최종 Archive를 시뮬레이션한다.
- 충돌 병합 직전 상태를 UID별 30분·1회 Undo 지점으로 보관한다.
- Undo 적용 전 현재 병합 상태를 일반 복구 지점으로 다시 보존한다.
- 복구 저장소를 v2로 올리고 이름 변경·검색·고정, 고정 3개+최근 5개 보관을 추가했다.
- v1 복구 JSON 마이그레이션과 UID 격리를 실행 하네스로 검증했다.
- 실제 물리 기기 캡처, 최종 수작업 공격·장비 원화, 실제 npm 의존성 기반 빌드는 완료로 기록하지 않았다.

## 2026-07-31 · v1.11.27

- 두 외형 복구 지점을 실제 캐릭터 카드로 비교하는 `CharacterAppearanceRecoveryCompareScene`을 추가했다.
- 슬롯·순서·고정·최근 프리셋 변경을 `lumerift-character-appearance-diff-v1`으로 계산한다.
- 복구·Cloud 병합·Undo 작업을 UID별 최대 100건의 감사 기록으로 남긴다.
- 비교 차이와 관련 감사 기록, 전체 감사 기록을 JSON으로 내보낼 수 있다.
- 신규 런타임 이미지는 추가하지 않았고 Player Save v4·Archive v3·App Check 비활성 정책을 유지했다.
- 실제 물리 기기 캡처, 최종 수작업 공격·장비 원화, 실제 npm 의존성 기반 빌드는 완료로 기록하지 않았다.

## 2026-08-03 · v1.11.28

- 사용자 승인 캐릭터·몬스터 쇼케이스 2종을 Premium Art Direction v2 기준선으로 등록
- 에셋 보관소 Lazy Reference 번들 연결, 초기 번들 제외
- 캐릭터·몬스터·UI·스킬 VFX·장비 품질 게이트 추가
- 941×1672 WebP 런타임 감수본과 제작용 마스터 분리
- 실제 전투 Atlas 전체 교체는 완료로 기록하지 않음


## 2026-08-03 · v1.11.29

- Premium Art Direction v2 기반 플레이어 금속 트림·얼굴 크레스트·흉부 룬·망토 윤곽을 전투에 연결했다.
- 3개 무기 계열·8방향·10행동·6레이어, 계획 본체 1,752프레임의 제작 계약을 추가했다.
- 엘리트·보스 왕관형 크리스털·약점 코어·발톱 실루엣을 추가하고 총 936프레임 Atlas 계획을 등록했다.
- Light·Heavy·Ultimate 룬 VFX와 공통 Premium UI Frame v3를 적용했다.
- 신규 런타임 이미지는 0개이며 최종 수작업 Atlas 완료로 기록하지 않았다.


## 2026-08-03 · v1.11.30

- 플레이어 얼굴·전후면 헤어·갑주 플레이트·트림·망토 원단·룬 코어를 독립 프로그램 레이어로 고도화했다.
- 검·대검·균열 장창의 공격 윤곽과 잔상을 정밀 연격·중량 절단·직선 관통으로 분리했다.
- 캐릭터 스튜디오와 전투가 동일한 Premium Character Runtime v2를 사용한다.
- Void Warden·Lumen Mender·Abyssal Harbinger의 엘리트·보스 외형과 페이즈 파편을 개별화했다.
- Light·Heavy·Ultimate 룬 스파크와 UI Frame v3.1을 적용했다.
- 신규 런타임 이미지는 0개이며 최종 수작업 Atlas 완료로 기록하지 않았다.


## 2026-08-03 · v1.11.31
- Premium HUD v15 WebP Atlas 8프레임을 전투·보스 HUD·인벤토리에 적용했다.
- 보스 코어 라이프사이클을 본체와 HUD 공통 계약으로 연결했다.
- 캡처 증빙을 JSON + 실제 이미지 SHA-256/바이트/해상도 검증 방식으로 강화했다.
- 최종 수작업 본체 Atlas와 물리 기기 승인은 미완료 상태로 유지했다.


## 2026-08-03 · v1.11.32

- 플레이어 얼굴·헤어·갑주·망토·룬·무기 투명 WebP 파츠 Atlas 16프레임을 전투와 캐릭터 스튜디오에 연결했다.
- Void·Frost·Inferno 엘리트와 Abyssal 보스 파츠 Atlas 16프레임을 기존 몬스터 본체 위에 합성했다.
- 보스 코어 5상태 12프레임 FX와 Premium UI Icons 16프레임을 적용했다.
- v16 Atlas 로딩 실패 시 기존 v15 HUD·v10/v11 본체·프로그램 레이어 폴백을 유지한다.
- 최종 전신 수작업 Atlas와 Android/iOS 물리 기기 승인은 완료로 기록하지 않았다.
